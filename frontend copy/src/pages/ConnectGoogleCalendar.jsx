...
const ConnectGoogleCalendar = () => {
  ...
  if (connected === null) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[var(--pl-bg)] via-[var(--pl-surface-strong)] to-[var(--pl-bg)] flex items-center justify-center p-6">
        <div className="bg-[var(--pl-surface-strong)] rounded-xl shadow-lg p-8 max-w-md w-full text-center">
          <FaSpinner className="animate-spin text-4xl text-[var(--pl-primary)] mx-auto mb-4" />
          <h2 className="text-xl font-semibold text-[var(--pl-text)] mb-2">Checking Calendar Connection</h2>
          <p className="text-[var(--pl-text-secondary)]">Please wait...</p>
        </div>
      </div>
    );
  }
  ...
  if (connected === false) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[var(--pl-bg)] via-[var(--pl-surface-strong)] to-[var(--pl-bg)] flex items-center justify-center p-6">
        <div className="bg-[var(--pl-surface-strong)] rounded-xl shadow-lg p-8 max-w-md w-full">
          {/* Error Message */}
          {errorMessage && (
            <div className="mb-6 p-4 bg-[var(--pl-danger)]/10 border-l-4 border-[var(--pl-danger)] rounded-lg relative">
              <button
                onClick={() => setErrorMessage(null)}
                className="absolute top-2 right-2 p-1 hover:bg-[var(--pl-danger)]/20 rounded transition-colors"
                title="Dismiss"
              >
                <FaTimes className="text-[var(--pl-danger)] text-sm" />
              </button>
              <div className="flex items-start gap-3 pr-6">
                <FaExclamationTriangle className="text-[var(--pl-danger)] text-xl flex-shrink-0 mt-0.5" />
                <div className="flex-1">
                  <p className="text-sm font-medium text-[var(--pl-danger)]">{errorMessage}</p>
                  <button
                    onClick={() => setErrorMessage(null)}
                    className="mt-3 px-4 py-2 bg-[var(--pl-danger)] text-white rounded-lg hover:bg-[var(--pl-danger)]/90 transition-colors text-sm font-medium"
                  >
                    OK
                  </button>
                </div>
          ...
          <div className="text-center mb-6">
            <div className="inline-flex items-center justify-center w-16 h-16 bg-[var(--pl-primary)]/20 rounded-full mb-4">
              <FaCalendar className="text-3xl text-[var(--pl-primary)]" />
            </div>
            <h1 className="text-3xl font-bold text-[var(--pl-text)] mb-2">Connect Google Calendar</h1>
            <p className="text-[var(--pl-text-secondary)] mb-2">
              Connect your Google Calendar to view and manage your events in one place.
            </p>
            {registeredEmail && (
              <p className="text-sm text-[var(--pl-text-muted)]">
                <span className="font-medium">Important:</span> You must connect using the same email address you registered with: <span className="font-semibold text-[var(--pl-text)]">{registeredEmail}</span>
              </p>
            )}
          </div>
          ...
          <button
            ...
            disabled={connecting}
            className="w-full py-3 px-6 bg-[var(--pl-primary)] text-white rounded-lg hover:bg-[var(--pl-link-hover)] disabled:bg-[var(--pl-disabled)] disabled:cursor-not-allowed flex items-center justify-center gap-3 font-semibold transition-colors"
          >
            {connecting ? (
              <>
          ...
          <p className="text-xs text-[var(--pl-text-muted)] text-center mt-4">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[var(--pl-bg)] via-[var(--pl-surface-strong)] to-[var(--pl-bg)] p-6">
      <div className="max-w-7xl mx-auto">
        {/* Error Message Banner */}
        {errorMessage && (
          <div className="mb-6 p-4 bg-[var(--pl-danger)]/10 border-l-4 border-[var(--pl-danger)] rounded-lg relative shadow-md">
            <button
              onClick={() => setErrorMessage(null)}
              className="absolute top-2 right-2 p-1 hover:bg-[var(--pl-danger)]/20 rounded transition-colors"
              title="Dismiss"
