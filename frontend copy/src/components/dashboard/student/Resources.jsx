...
export default function PlacementResources() {
  ...
  return (
    <div className="min-h-screen bg-[var(--pl-bg)] p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        <section className="bg-[var(--pl-surface-strong)] rounded-3xl shadow-lg p-8 border border-[var(--pl-border)]">
          <header className="mb-6">
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--pl-primary)]">
              Placement resources
            </p>
            <h1 className="text-3xl font-bold text-[var(--pl-text)] mt-2">
              AI-Powered Placement Guidance
            </h1>
            <p className="text-[var(--pl-text-secondary)] mt-2">
          ...
          <form onSubmit={handleGenerate} className="relative">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
              <FaMagic className="h-5 w-5 text-[var(--pl-primary)]/60" />
            </div>
            <input
              type="text"
              value={query}
              ...
              className="w-full pl-12 pr-32 py-4 rounded-2xl border border-[var(--pl-border)] focus:outline-none focus:ring-2 focus:ring-[var(--pl-primary)] focus:border-[var(--pl-primary)] text-[var(--pl-text)]"
              maxLength={500}
            />
            <button
              type="submit"
              className="absolute inset-y-2 right-2 px-5 py-2 rounded-xl bg-[var(--pl-primary)] text-white font-semibold disabled:opacity-60 disabled:cursor-not-allowed hover:bg-[var(--pl-link-hover)] transition-all"
              disabled={loading}
            >
          ...
          {error && (
            <div className="mt-4 p-4 bg-[var(--pl-danger)]/10 border border-[var(--pl-danger)]/30 rounded-xl">
              <p className="text-sm text-[var(--pl-danger)]">{error}</p>
            </div>
          )}
          
          {!error && loading && (
            <div className="mt-4 p-4 bg-[var(--pl-primary)]/10 border border-[var(--pl-primary)]/30 rounded-xl">
              <p className="text-sm text-[var(--pl-primary)] font-medium">
        ...
        {!hasSearched && (
          <div className="bg-[var(--pl-surface-strong)] rounded-3xl border border-dashed border-[var(--pl-border)] p-10 text-center text-[var(--pl-text-muted)]">
            <FaMagic className="h-12 w-12 mx-auto mb-4 text-[var(--pl-text-muted)]" />
            <p className="text-lg font-medium">Ask any placement-related question</p>
            <p className="text-sm mt-2 text-[var(--pl-text-muted)]">
