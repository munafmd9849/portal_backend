...
const StudentQuerySystem = () => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'resolved':
        return <FaCheck className="text-[var(--pl-success)]" />;
      case 'rejected':
        return <FaTimesCircle className="text-[var(--pl-danger)]" />;
      case 'under_review':
        return <FaClock className="text-[var(--pl-primary)]" />;
      default:
        return <FaClock className="text-[var(--pl-text-muted)]" />;
    }
  };
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'resolved':
        return 'bg-[var(--pl-success)]/20 text-[var(--pl-success)]';
      case 'rejected':
        return 'bg-[var(--pl-danger)]/20 text-[var(--pl-danger)]';
      case 'under_review':
        return 'bg-[var(--pl-primary)]/20 text-[var(--pl-primary)]';
      default:
        return 'bg-[var(--pl-surface)] text-[var(--pl-text)]';
    }
  };
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50/30 flex items-center justify-center p-4">
        <div className="bg-[var(--pl-surface-strong)] rounded-2xl shadow-lg p-8 max-w-md w-full border border-[var(--pl-border)]">
          <div className="text-center">
            <div className="bg-[var(--pl-success)]/20 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6">
              <FaCheckCircle className="text-[var(--pl-success)] text-3xl" />
            </div>
            <h2 className="text-2xl font-bold text-[var(--pl-text)] mb-2">Query Submitted Successfully!</h2>
            ...
            <div className="bg-[var(--pl-primary)]/10 rounded-xl p-4 mb-6 text-left border border-[var(--pl-primary)]/30">
              <h3 className="font-medium text-[var(--pl-primary)] mb-2">Reference ID: #{referenceId}</h3>
              <p className="text-sm text-[var(--pl-primary)]">Keep this reference ID for future communication.</p>
            </div>
            <div className="flex gap-3">
              <button
                ...
                className="px-4 py-3 bg-[var(--pl-surface)] text-[var(--pl-text)] font-medium rounded-xl hover:bg-[var(--pl-surface)]/80 transition-all duration-200 flex-1 flex items-center justify-center"
              >
              ...
              <button
                onClick={resetForm}
                className="px-6 py-3 bg-[var(--pl-primary)] text-white font-medium rounded-xl hover:bg-[var(--pl-link-hover)] transition-all duration-200 shadow-md hover:shadow-lg flex-1"
              >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[var(--pl-bg)] to-[var(--pl-primary)]/10 p-4 py-8">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10">
          <h1 className="text-3xl font-bold text-[var(--pl-text)] mb-3">Student Query Portal</h1>
          <p className="text-[var(--pl-text-secondary)] max-w-2xl mx-auto">
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-[var(--pl-surface-strong)] rounded-sm p-1 shadow-sm border border-[var(--pl-border)] inline-flex">
            <button
              onClick={() => setActiveView('new')}
              className={`px-6 py-3 rounded-md font-medium transition-all duration-200 ${activeView === 'new' ? 'bg-[var(--pl-accent-orange)]/30 text-[var(--pl-text)] shadow-md' : 'text-[var(--pl-text-secondary)] hover:text-[var(--pl-text)]'}`}
            >
              New Query
            </button>
            <button
              onClick={() => setActiveView('history')}
              className={`px-6 py-3 rounded-md font-medium transition-all duration-200 flex items-center ${activeView === 'history' ? 'bg-[var(--pl-accent-orange)] text-white shadow-md' : 'text-[var(--pl-text-secondary)] hover:text-[var(--pl-text)]'}`}
            >
              <FaHistory className="mr-2" />
              Query History
            </button>
        ...
        {activeView === 'history' ? (
          /* Query History View */
          <div className="bg-[var(--pl-surface-strong)] rounded-2xl shadow-lg overflow-hidden border border-[var(--pl-border)]">
            <div className="p-6 border-b border-[var(--pl-border)]">
              <h2 className="text-xl font-bold text-[var(--pl-text)]">Your Query History</h2>
              <p className="text-[var(--pl-text-secondary)]">Track the status of your previous queries</p>
            </div>
            
            <div className="p-6">
              {loadingQueries ? (
                <div className="text-center py-10">
                  <div className="bg-[var(--pl-primary)]/20 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                    <FaClock className="text-[var(--pl-primary)] text-2xl animate-spin" />
                  </div>
                  <h3 className="text-lg font-medium text-[var(--pl-text)] mb-2">Loading your queries...</h3>
                  <p className="text-[var(--pl-text-muted)]">Please wait while we fetch your query history.</p>
                </div>
              ) : pastQueries.length === 0 ? (
                <div className="text-center py-10">
                  <div className="bg-[var(--pl-surface)] w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                    <FaHistory className="text-[var(--pl-text-muted)] text-2xl" />
                  </div>
                  <h3 className="text-lg font-medium text-[var(--pl-text)] mb-2">No queries yet</h3>
                  <p className="text-[var(--pl-text-muted)] mb-4">You haven't submitted any queries to the placement cell.</p>
                  <button
                    onClick={() => setActiveView('new')}
                    className="px-4 py-2 bg-[var(--pl-primary)] text-white rounded-xl hover:bg-[var(--pl-link-hover)] transition-colors"
                  >
                ...
                <div className="space-y-4">
                  {pastQueries.map(query => (
                    <div key={query.id} className="border border-[var(--pl-border)] rounded-xl overflow-hidden">
                      <div 
                        className="p-4 bg-[var(--pl-surface)] flex justify-between items-center cursor-pointer hover:bg-[var(--pl-surface)]/80 transition-colors"
                        onClick={() => toggleQueryExpand(query.id)}
                      >
                        <div className="flex items-center">
                          <div className="mr-4">
                            {query.type === 'question' && <FaQuestionCircle className="text-[var(--pl-primary)] text-xl" />}
                            {query.type === 'cgpa' && <FaChartLine className="text-[var(--pl-success)] text-xl" />}
                            {query.type === 'backlog' && <FaChartLine className="text-[var(--pl-accent-orange)] text-xl" />}
                            {query.type === 'calendar' && <FaCalendarAlt className="text-[var(--pl-accent-purple)] text-xl" />}
                          </div>
                          <div>
                            <h3 className="font-medium text-[var(--pl-text)]">{query.subject}</h3>
                        ...
                        <div className="flex items-center">
                          ...
                          {expandedQuery === query.id ? <FaChevronUp className="text-[var(--pl-text-muted)]" /> : <FaChevronDown className="text-[var(--pl-text-muted)]" />}
                        </div>
                      </div>
                      
                      {expandedQuery === query.id && (
                        <div className="p-4 bg-[var(--pl-surface-strong)] border-t border-[var(--pl-border)]">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
                            <div>
                              <h4 className="text-sm font-medium text-[var(--pl-text-muted)] mb-1">Query Type</h4>
                              <p className="capitalize">{query.type}</p>
                            </div>
                            <div>
                              <h4 className="text-sm font-medium text-[var(--pl-text-muted)] mb-1">Status</h4>
                              <div className="flex items-center">
                            ...
                            <div>
                              <h4 className="text-sm font-medium text-[var(--pl-text-muted)] mb-1">Date Submitted</h4>
                          ...
                          {query.type === 'question' && (
                            <div className="mb-4">
                              {query.jobId && (
                                <div className="mb-3 p-3 bg-[var(--pl-primary)]/10 border border-[var(--pl-primary)]/30 rounded-lg">
                                  <h4 className="text-sm font-medium text-[var(--pl-primary)] mb-1">Job Posting</h4>
                                  <p className="text-sm text-[var(--pl-primary)]">
                                    {query.subject.includes('about') ? query.subject.split('about')[1]?.trim() : query.subject}
                                  </p>
                                </div>
                              )}
                              <h4 className="text-sm font-medium text-[var(--pl-text-muted)] mb-1">Your Question</h4>
                              <p className="text-[var(--pl-text)]">{query.message}</p>
                            </div>
                          )}
                          
                          {query.type === 'cgpa' && (
                            <div className="mb-4">
                              <h4 className="text-sm font-medium text-[var(--pl-text-muted)] mb-1">CGPA Submitted</h4>
                              <p className="text-[var(--pl-text)]">{query.cgpa}</p>
                            </div>
                          )}
                          
                          {query.type === 'backlog' && (
                            <div className="mb-4">
                              <h4 className="text-sm font-medium text-[var(--pl-text-muted)] mb-1">Backlogs Submitted</h4>
                              <p className="text-[var(--pl-text)]">{query.backlogs || query.metadata?.backlogs || 'N/A'}</p>
                            </div>
                          )}
                          ...
                          {query.type === 'calendar' && (
                            <div className="mb-4">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                  <h4 className="text-sm font-medium text-[var(--pl-text-muted)] mb-1">From</h4>
                                  <p className="text-[var(--pl-text)]">{new Date(query.startDate).toLocaleDateString()}</p>
                                </div>
                                <div>
                                  <h4 className="text-sm font-medium text-[var(--pl-text-muted)] mb-1">To</h4>
                                  <p className="text-[var(--pl-text)]">{new Date(query.endDate).toLocaleDateString()}</p>
                                </div>
                                <div>
                                  <h4 className="text-sm font-medium text-[var(--pl-text-muted)] mb-1">Time Slot</h4>
                                  <p className="text-[var(--pl-text)]">{query.timeSlot}</p>
                                </div>
                              </div>
                            </div>
                          )}
                          ...
                          {query.adminResponse && (
                            <div className="pt-4 border-t border-[var(--pl-border)]">
                              <h4 className="text-sm font-medium text-[var(--pl-text-muted)] mb-2">Admin Response</h4>
                              <div className="bg-[var(--pl-primary)]/10 rounded-lg p-4 border border-[var(--pl-primary)]/30">
                                <p className="text-[var(--pl-primary)]">{query.adminResponse}</p>
                              </div>
                            </div>
                          )}
                        </div>
          ...
          <div className="bg-[var(--pl-surface-strong)] rounded-2xl shadow-lg overflow-hidden border border-[var(--pl-border)]">
            ...
            <div className="border-b border-[var(--pl-border)] bg-[var(--pl-surface)]/50">
                  ...
                  <button
                    ...
                    className={`px-5 py-4 flex flex-col items-center min-w-[140px] border-b-2 transition-all duration-200 ${
                      activeTab === type.id
                        ...
                        : 'border-transparent text-[var(--pl-text-secondary)] hover:text-[var(--pl-text)]'
                    }`}
                  >
                    <span className={`text-lg mb-2 ${activeTab === type.id ? `text-[var(--pl-primary)]` : 'text-[var(--pl-text-muted)]'}`}>
                      {type.icon}
                    </span>
                    <span className="font-medium text-sm">{type.name}</span>
                    <span className="text-xs mt-1 text-[var(--pl-text-muted)]">{type.description}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6">
              ...
              {activeTab === 'question' ? (
                <div className="mb-6">
                  <label className="block text-sm font-semibold text-[var(--pl-text)] mb-2 flex items-center gap-2">
                    <FaBriefcase className="w-4 h-4 text-[var(--pl-primary)]" />
                    Select Job Posting <span className="text-red-500">*</span>
                  </label>
                  {loadingJobs ? (
                    <div className="w-full px-4 py-3 border-2 border-[var(--pl-border)] rounded-lg bg-[var(--pl-surface)] flex items-center justify-center">
                      <FaClock className="animate-spin text-[var(--pl-primary)] mr-2" />
                      <span className="text-[var(--pl-text-secondary)]">Loading jobs...</span>
                    </div>
                  ) : jobs.length === 0 ? (
                    <div className="w-full px-4 py-3 border-2 border-[var(--pl-warning)]/50 rounded-lg bg-[var(--pl-warning)]/10">
                      <p className="text-[var(--pl-warning)] text-sm">No job postings available at the moment. Please check back later.</p>
                    </div>
                  ) : (
                    <div className="relative" ref={jobSelectorRef}>
                      <button
                        ...
                        className={`w-full border-2 ${formErrors.selectedJobId ? 'border-red-500' : 'border-[var(--pl-border)]'} rounded-lg px-4 py-3 text-sm text-left flex items-center justify-between transition-all duration-200 bg-[var(--pl-surface-strong)] hover:border-[var(--pl-primary)] hover:bg-[var(--pl-primary)]/10 hover:shadow-sm focus:border-[var(--pl-primary)] focus:ring-2 focus:ring-[var(--pl-primary)] focus:outline-none cursor-pointer`}
                      >
                        <span className="truncate flex-1 text-[var(--pl-text)]">
                        ...
                        <FaChevronDown className={`w-3 h-3 text-[var(--pl-text-muted)] flex-shrink-0 transition-transform duration-200 ${showJobSelector ? 'rotate-180' : ''}`} />
                      </button>
                      
                      {showJobSelector && (
                        <div className="absolute z-20 w-full bg-[var(--pl-surface-strong)] border-2 border-[var(--pl-border)] rounded-lg shadow-lg mt-1 max-h-96 overflow-y-auto">
                              ...
                              return (
                                <div
                                  ...
                                  className={`p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 ${
                                    isSelected 
                                      ? 'border-[var(--pl-primary)] bg-[var(--pl-primary)]/10 shadow-md' 
                                      : 'border-[var(--pl-border)] hover:border-[var(--pl-primary)]/50 hover:bg-[var(--pl-primary)]/5'
                                  }`}
                                >
                                  <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                      <h4 className={`font-semibold text-sm mb-1 ${isSelected ? 'text-[var(--pl-primary)]' : 'text-[var(--pl-text)]'}`}>
                                        {job.jobTitle}
                                      </h4>
                                      <p className={`text-xs mb-2 ${isSelected ? 'text-[var(--pl-primary)]' : 'text-[var(--pl-text-secondary)]'}`}>
                                        {job.companyName || job.company}
                                        {job.companyLocation && ` • ${job.companyLocation}`}
                                      </p>
                                      <div className="flex flex-wrap gap-2 mt-2">
                                        {job.jobType && (
                                          <span className={`px-2 py-1 text-xs rounded ${isSelected ? 'bg-[var(--pl-primary)]/20 text-[var(--pl-primary)]' : 'bg-[var(--pl-surface)] text-[var(--pl-text)]'}`}>
                                            {job.jobType}
                                          </span>
                                        )}
                                        {job.workMode && (
                                    ...
                                    {isSelected && (
                                      <FaCheckCircle className="w-5 h-5 text-[var(--pl-primary)] flex-shrink-0 ml-2" />
                                    )}
                                  </div>
                                </div>
                              );
                ...
                <div className="mb-6">
                  <label className="block text-[var(--pl-text)] font-medium mb-2">Subject</label>
                  <input
                    type="text"
                    name="subject"
                    ...
                    className={`w-full px-4 py-3 border ${formErrors.subject ? 'border-red-500' : 'border-[var(--pl-border)]'} rounded-xl focus:outline-none focus:ring-2 focus:ring-[var(--pl-primary)] focus:border-transparent transition-all duration-100`}
                    required
                  />
              ...
              {activeTab === 'question' && (
                <div className="mb-6">
                  <label className="block text-[var(--pl-text)] font-medium mb-2">Your Question</label>
                  <textarea
                    name="message"
                    ...
                    rows={4}
                    className={`w-full px-4 py-3 border min-h-[120px] max-h-[300px] resize-y ${formErrors.message ? 'border-red-500' : 'border-[var(--pl-border)]'} rounded-xl focus:outline-none focus:ring-2 focus:ring-[var(--pl-primary)] focus:border-transparent`}
                    required
                  />
