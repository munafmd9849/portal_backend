/**
 * Recruiter Calendar Component
 * Displays interviews, drive dates, schedule updates, and other important events
 */
...
const RecruiterCalendar = () => {
  ...
  const loadEvents = async () => {
    try {
      ...
      // const interviewsData = await api.getRecruiterInterviews();
  ...
  const events = useMemo(() => {
    ...
    interviews.forEach(interview => {
      if (interview.date) {
        ...
        eventMap[dateKey].push({
          ...
          job: interview.jobTitle || 'Unknown Job',
    ...
    drives.forEach(drive => {
      if (drive.driveDate) {
        ...
        eventMap[dateKey].push({
          type: 'drive',
          title: `Drive: ${drive.jobTitle || 'Job Drive'}`,
          company: drive.companyName || 'Your Company',
    ...
    scheduleUpdates.forEach(update => {
      if (update.updateDate) {
        ...
        eventMap[dateKey].push({
          type: 'update',
          title: `Schedule Update: ${update.title || 'Update'}`,
    ...
    return eventMap;
  }, [interviews, drives, scheduleUpdates]);
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        {event.job && (
                          <p className="text-sm text-gray-600 mt-1">Job: {event.job}</p>
                        )}
                        {event.company && (
                          <p className="text-sm text-gray-600 mt-1">Company: {event.company}</p>
                        )}
                        {event.time && (
