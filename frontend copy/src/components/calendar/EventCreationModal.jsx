...
const EventCreationModal = ({ isOpen, onClose, onSuccess, userRole, selectedDate = null, onReconnectRequest }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-[var(--pl-surface-strong)] rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-[var(--pl-border)]">
          <h2 className="text-2xl font-bold text-[var(--pl-text)]">Create New Event</h2>
          <button
            onClick={onClose}
            className="p-2 hover:bg-[var(--pl-surface)] rounded-lg transition-colors"
          >
            <X className="h-5 w-5 text-[var(--pl-text-secondary)]" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          {error && (
            <div className={`p-4 border rounded-lg ${
              error.includes('read-only') || error.includes('Insufficient') 
                ? 'bg-[var(--pl-warning)]/10 border-[var(--pl-warning)]/30' 
                : 'bg-[var(--pl-danger)]/10 border-[var(--pl-danger)]/30'
            }`}>
              ...
              {(error.includes('read-only') || error.includes('Insufficient')) && (
                <div className="mt-3 flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 bg-[var(--pl-warning)] text-white rounded-lg hover:bg-[var(--pl-warning)]/90 transition-colors text-sm font-medium"
                  >
                  ...
                  <button
                    type="button"
                    onClick={onClose}
                    className="px-4 py-2 bg-[var(--pl-surface)] text-[var(--pl-text)] rounded-lg hover:bg-[var(--pl-surface)]/80 transition-colors text-sm"
                  >
                    Cancel
                  </button>
                </div>
          ...
          {/* Title */}
          <div>
            <label className="block text-sm font-medium text-[var(--pl-text)] mb-2">
            ...
            <input
              ...
              required
              className="w-full px-4 py-2 border border-[var(--pl-border)] rounded-lg focus:ring-2 focus:ring-[var(--pl-primary)] focus:border-transparent"
          ...
          <div>
            ...
            {(userRole === 'ADMIN' || userRole === 'SUPER_ADMIN') && (
              <div className="mb-4">
                <button
                  ...
                  className="w-full px-4 py-2 bg-[var(--pl-primary)]/10 text-[var(--pl-primary)] rounded-lg hover:bg-[var(--pl-primary)]/20 transition-colors flex items-center justify-between"
                >
                  <span className="flex items-center gap-2">
                    <Users className="h-4 w-4" />
                    Select Students from Portal
                    {selectedStudents.length > 0 && (
                      <span className="px-2 py-0.5 bg-[var(--pl-primary)]/20 rounded-full text-xs">
                ...
                {showStudentSelector && (
                  <div className="mt-2 border border-[var(--pl-border)] rounded-lg bg-[var(--pl-surface-strong)] max-h-64 overflow-hidden flex flex-col">
                    {/* Search */}
                    <div className="p-3 border-b border-[var(--pl-border)]">
                      <div className="relative">
                        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-[var(--pl-text-muted)]" />
                        <input
                          type="text"
                          value={studentSearch}
                          onChange={(e) => setStudentSearch(e.target.value)}
                          placeholder="Search by name, email, or enrollment ID..."
                          className="w-full pl-10 pr-4 py-2 border border-[var(--pl-border)] rounded-lg focus:ring-2 focus:ring-[var(--pl-primary)] focus:border-transparent"
                        />
                      </div>
                    </div>
                    ...
                    <div className="overflow-y-auto flex-1">
                      {loadingStudents ? (
                        <div className="p-4 text-center text-[var(--pl-text-muted)]">Loading students...</div>
                          ...
                          {filteredStudents.map((student) => {
                            ...
                            return (
                              <button
                                ...
                                className={`w-full px-4 py-3 text-left hover:bg-[var(--pl-surface)] transition-colors flex items-center justify-between ${
                                  isSelected ? 'bg-[var(--pl-primary)]/10' : ''
                                }`}
                              >
                                <div className="flex-1 min-w-0">
                                  <div className="font-medium text-[var(--pl-text)] truncate">
                                    {student.fullName || 'Unknown'}
                                  </div>
                                  <div className="text-sm text-[var(--pl-text-secondary)] truncate">
                                    {student.email}
                                  </div>
                                ...
                                {isSelected && (
                                  <Check className="h-5 w-5 text-[var(--pl-primary)] flex-shrink-0 ml-2" />
                                )}
                              </button>
                            );
                          })}
            ...
            {/* Manual Email Input */}
            <div>
              <label className="block text-xs font-medium text-[var(--pl-text-secondary)] mb-2">
              ...
              <div className="flex gap-2 mb-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 border border-[var(--pl-border)] rounded-lg focus:ring-2 focus:ring-[var(--pl-primary)] focus:border-transparent"
                  placeholder="Enter email address"
                />
                <button
                  type="button"
                  onClick={handleAddAttendee}
                  className="px-4 py-2 bg-[var(--pl-surface)] text-[var(--pl-text)] rounded-lg hover:bg-[var(--pl-surface)]/80 transition-colors"
                >
                  Add
                </button>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.attendeesEmails.map((email) => {
                    ...
                    return (
                      <span
                        key={email}
                        className="inline-flex items-center gap-2 px-3 py-1 bg-[var(--pl-primary)]/20 text-[var(--pl-primary)] rounded-full text-sm"
                      >
                        ...
                        <button
                          ...
                          className="hover:text-[var(--pl-link-hover)]"
                        >
                          <X className="h-3 w-3" />
                        </button>
                      </span>
          ...
          <div className="flex items-center">
            <input
              ...
              onChange={handleInputChange}
              className="h-4 w-4 text-[var(--pl-primary)] focus:ring-[var(--pl-primary)] border-[var(--pl-border)] rounded"
            />
            <label htmlFor="meetLink" className="ml-2 text-sm text-[var(--pl-text)]">
              Create Google Meet link for this event
            </label>
          </div>

          {/* Actions */}
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-[var(--pl-border)]">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-[var(--pl-text)] bg-[var(--pl-surface)] rounded-lg hover:bg-[var(--pl-surface)]/80 transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2 bg-[var(--pl-primary)] text-white rounded-lg hover:bg-[var(--pl-link-hover)] disabled:bg-[var(--pl-disabled)] disabled:cursor-not-allowed transition-colors font-medium"
            >
