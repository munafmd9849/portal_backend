...
const OurPartners = () => {
  ...
  const handleCardToggle = (rowKey, partnerId, index) => {
    const key = `${rowKey}-${partnerId}-${index}`;
    setActiveCardKey((prev) => (prev === key ? null : key));

    // Pause the row briefly so tap UI is readable on mobile
    if (rowKey === "row1") setPausedRow1(true);
    if (rowKey === "row2") setPausedRow2(true);

    if (activeTimeoutRef.current) window.clearTimeout(activeTimeoutRef.current);
    activeTimeoutRef.current = window.setTimeout(() => {
      setActiveCardKey(null);
      if (rowKey === "row1") setPausedRow1(false);
      if (rowKey === "row2") setPausedRow2(false);
    }, 2200);
  };
  ...
  return (
    <section
      id="our-partners"
      className="py-16 overflow-hidden relative bg-[var(--pl-bg)]"
    >
      {/* Section Heading */}
      <div className="text-center mb-14 px-4">
        <h2 className="text-balance text-3xl sm:text-4xl lg:text-5xl font-bold text-[var(--pl-text)] mb-4 tracking-tight leading-tight">
          Our <span className="text-[var(--pl-primary)]">Partners</span> in Launching Careers
        </h2>
        <p className="text-base sm:text-lg text-[var(--pl-text-secondary)] max-w-3xl mx-auto">
          Leading companies trust us to deliver exceptional talent
        </p>
      </div>
      
      {/* Animated rows (desktop + mobile/tablet) */}
      <div className="w-full overflow-hidden relative py-4">
        <div className="space-y-6">
          ...
          <div
            ...
            onTouchStart={() => setPausedRow1(true)}
            onTouchEnd={() => setPausedRow1(false)}
          >
            {duplicatedRow1.map((partner, index) => {
              const cardKey = `row1-${partner.id}-${index}`;
              const isActive = activeCardKey === cardKey;
              return (
                <div
                  key={`1-${partner.id}-${index}`}
                  className="w-44 h-24 sm:w-48 sm:h-28 bg-[var(--pl-surface-strong)] rounded-2xl border border-[var(--pl-border)] shadow-sm flex items-center justify-center relative overflow-hidden flex-shrink-0 transition-all duration-300 ease-out hover:-translate-y-0.5 hover:shadow-lg group cursor-pointer select-none"
                  role="button"
                  tabIndex={0}
                  onClick={() => handleCardToggle("row1", partner.id, index)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") handleCardToggle("row1", partner.id, index);
                  }}
                >
                  <div
                    className={`w-4/5 h-4/5 flex items-center justify-center transition-all duration-300 ease-out z-10 ${
                  ...
                  <div
                    className={`absolute inset-0 bg-[linear-gradient(135deg,color-mix(in_oklab,var(--pl-primary)_10%,white),white)] flex flex-col items-center justify-center p-4 sm:p-6 opacity-0 transition-opacity duration-200 ease-out text-[var(--pl-text)] text-center ${
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center">
                      <span className="text-xl sm:text-2xl font-bold leading-none text-[var(--pl-primary)]">
                        {partner.studentsPlaced}+
                      </span>
                      <span className="text-sm text-[var(--pl-text-secondary)] mt-1">Students Placed</span>
                    </div>
                  </div>
                </div>
              );
          ...
          <div
            ...
            onTouchStart={() => setPausedRow2(true)}
            onTouchEnd={() => setPausedRow2(false)}
          >
            {duplicatedRow2.map((partner, index) => {
              const cardKey = `row2-${partner.id}-${index}`;
              const isActive = activeCardKey === cardKey;
              return (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => handleCardToggle("row2", partner.id, index)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") handleCardToggle("row2", partner.id, index);
                  }}
                >
                  <div
                    className={`w-4/5 h-4/5 flex items-center justify-center transition-all duration-300 ease-out z-10 ${
