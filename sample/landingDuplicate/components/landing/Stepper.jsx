...
export default function Stepper({
  ...
  onNext,
  // Controls the max-width of the stepper container.
  // Default keeps current behavior; pass e.g. "max-w-none" for full-width.
      ...
      >
        ...
        {!isCompleted && (
          <div className={`px-6 pb-6 ${footerClassName}`}>
            <div
              className={`mt-8 flex ${currentStep !== 1 ? "justify-between" : "justify-end"} ${footerInnerClassName}`}
            >
              {currentStep !== 1 && (
                <button
                  onClick={handleBack}
              ...
              <button
                ...
                className={`flex items-center justify-center rounded-full py-2 px-4 text-sm font-semibold tracking-tight text-white transition ${
                  isNextDisabled
                    ? "bg-[var(--pl-primary)]/50 cursor-not-allowed"
                    : "bg-[var(--pl-primary)] hover:bg-[var(--pl-link-hover)] active:bg-[var(--pl-link-hover)]"
                }`}
                {...nextButtonProps}
                type="button"
              >
    ...
    >
      <motion.div
        variants={{
          inactive: {
            scale: 1,
            backgroundColor: "color-mix(in oklab, var(--pl-bg) 65%, var(--pl-text-muted))",
            color: "var(--pl-text)",
          },
          active: { scale: 1, backgroundColor: "var(--pl-primary)", color: "#ffffff" },
          complete: { scale: 1, backgroundColor: "var(--pl-primary)", color: "#ffffff" },
        }}
...
function StepConnector({ isComplete }) {
  const lineVariants = {
    ...
    complete: { width: "100%", backgroundColor: "var(--pl-primary)" },
  };

  return (
    <div className="relative mx-2 h-0.5 flex-1 overflow-hidden rounded bg-[var(--pl-border)]">
      <motion.div
