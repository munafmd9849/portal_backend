...
export default function CreateJob({ onCreated }) {
  ...
  useEffect(() => {
    if (!canCreateJobs) {
      ...
      // Show error message
      showError('Access Denied: Only ADMIN, SUPER_ADMIN, or RECRUITER users can create jobs.');
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">Only ADMIN, SUPER_ADMIN, or RECRUITER users can create jobs.</p>
  ...
  const handleSubmit = async (e) => {
    ...
    const userRole = role || user?.role;
    const allowedRoles = ['ADMIN', 'SUPER_ADMIN', 'RECRUITER'];
    const hasRequiredRole = canCreateJobs || (userRole && allowedRoles.includes(String(userRole).toUpperCase()));
