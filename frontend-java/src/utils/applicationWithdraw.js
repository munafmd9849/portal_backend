...
/** Screening states that always block withdrawal once reached. */
...
/** Student applications API wraps interview status in an object — normalize to a code. */
...
export function canStudentWithdrawApplication(application) {
  ...
  if (screening && screening !== 'APPLIED') {
    ...
    // No-gate jobs auto-set INTERVIEW_ELIGIBLE on apply — that is not real shortlisting.
