  ...
  const handleSave = async () => {
    try {
      ...
      await updatePlacementCompensation(row.applicationId, {
        offerCtc: offerCtc.trim() || null,
        offerStipend: offerStipend.trim() || null,
...
function PlacementRow({ row, onEdit, onOpenJob, onOpenStudent }) {
  return (
    <tr className="hover:bg-gray-50 transition-colors">
      ...
      <td className="px-4 py-3 text-sm text-gray-800 whitespace-nowrap">{row.offerCtc || '—'}</td>
      <td className="px-4 py-3 text-sm text-gray-800 whitespace-nowrap">{row.offerStipend || '—'}</td>
...
export default function PlacementsRegistry() {
  ...
  const view = searchParams.get('view') === 'student' ? 'student' : 'job';
  ...
  }, [search, school, center, batch, placementType, company, searchParams, toast]);
  ...
  const grouped = useMemo(() => {
    ...
    placements.forEach((p) => {
      const key = view === 'job' ? p.jobId : p.studentId;
      const label =
        view === 'job'
          ? `${p.jobTitle} · ${p.companyName}`
          : `${p.studentName} · ${p.batch || p.school || ''}`.trim();
      if (!map.has(key)) {
        map.set(key, { key, label, sub: view === 'job' ? p.companyName : p.studentEmail, rows: [] });
      }
  ...
  return (
    <div className="space-y-5 p-4 sm:p-6 max-w-[1600px] mx-auto overflow-x-hidden">
      <div className="flex justify-center mb-2">
        <div className="bg-white rounded-lg p-1 shadow-sm border border-gray-200 inline-flex flex-wrap justify-center gap-2">
          <button
            type="button"
            onClick={() => setView('job')}
            className={`px-4 sm:px-6 py-2 rounded-md font-medium transition-all duration-200 flex items-center gap-2 ${
              view === 'job' ? 'bg-blue-800 text-white shadow-md' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
            <Briefcase className="w-4 h-4" />
            By Job ({view === 'job' ? total : '…'})
          </button>
          <button
            type="button"
            onClick={() => setView('student')}
            className={`px-4 sm:px-6 py-2 rounded-md font-medium transition-all duration-200 flex items-center gap-2 ${
              view === 'student' ? 'bg-blue-800 text-white shadow-md' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
            <User className="w-4 h-4" />
            By Student ({view === 'student' ? total : '…'})
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-200 bg-gray-50 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-semibold text-gray-900">Joined Placements</h2>
        ...
        <div className="p-4 border-b border-gray-100 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-2">
          ...
          <input
            ...
            placeholder="Company"
            className="py-2 px-3 text-sm border border-gray-300 rounded-md focus:ring-1 focus:ring-blue-800"
          />
          <select
            value={school}
        ...
        {loading && placements.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100">
            {grouped.map((group) => (
              <div key={group.key}>
                <div className="px-4 py-2.5 bg-gray-50/80 border-b border-gray-100 flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-900 truncate">{group.label}</p>
                    {group.sub && <p className="text-xs text-gray-500 truncate">{group.sub}</p>}
                  </div>
                  <span className="text-xs font-medium text-blue-800 bg-blue-50 border border-blue-100 px-2 py-0.5 rounded shrink-0">
                    {group.rows.length}
                  </span>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[720px]">
                    <thead>
                      <tr className="text-left text-[10px] font-medium uppercase tracking-wider text-gray-500 border-b border-gray-100">
