    ...
    <div className="space-y-5 p-4 sm:p-6 max-w-[1600px] mx-auto">
            ...
            >
              <div className="flex items-center justify-between mb-4">
                 ...
                 <button 
                    ...
                    className="p-1.5 text-gray-400 hover:text-indigo-700 hover:bg-blue-50 rounded-md transition-colors opacity-0 group-hover:opacity-100"
                 >
              ...
              {!isAvailable ? (
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    ...
                    {slot.status === 'WAITING' && (
                      <button 
                        ...
                        className="py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md text-xs font-medium"
                      >
                        Start
                      </button>
                    )}
                    {(slot.status === 'LIVE' || slot.status === 'SCHEDULED' || slot.status === 'WAITING') && (
                      <button 
                        ...
                        className="py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md text-xs font-medium flex items-center justify-center gap-1.5"
                      >
                    ...
                    {slot.status === 'COMPLETED' && (
                      <button
                        ...
                        className="col-span-2 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md text-xs font-medium"
                      >
                        View feedback
                      </button>
                    )}
                ...
                <div className="flex-1 py-8 flex flex-col items-center justify-center border border-dashed border-gray-200 rounded-md bg-gray-50">
                  ...
                  <button 
                    ...
                    className="px-4 py-2 bg-indigo-600 text-white rounded-md text-xs font-medium hover:bg-indigo-700"
                  >
                    Assign candidate
                  </button>
                </div>
      ...
      {selectedSlot && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 overflow-y-auto">
          <div className="w-full max-w-2xl bg-white rounded-xl shadow-xl overflow-hidden border border-slate-200 relative my-auto">
            <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-white">
              <div>
                <h3 className="text-base font-semibold text-slate-900">Assign candidate</h3>
            ...
            <div className="p-5">
              <div className="relative mb-4">
                ...
                <input 
                  ...
                  className="w-full border border-gray-300 rounded-md pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-800 focus:border-indigo-600"
                  value={searchQuery}
                ...
                {filteredStudents.length === 0 ? (
                  ...
                  filteredStudents.map(student => (
                    ...
                    >
                      ...
                      <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-indigo-700 shrink-0" />
                    </button>
                  ))
                )}
              </div>
      ...
      {editingSlot && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 overflow-y-auto">
          <div className="w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden border border-slate-200 relative my-auto">
            <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-white">
              <h3 className="text-base font-semibold text-slate-900">Edit timing</h3>
              <button type="button" onClick={() => setEditingSlot(null)} className="p-2 rounded-lg text-slate-500 hover:bg-slate-100">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4">
               <div>
                  ...
                  <input 
                    type="datetime-local" 
                    className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-800 focus:border-indigo-600"
               ...
               <div className="flex gap-3 pt-2">
                  ...
                  <button 
                    ...
                    className="flex-1 py-2 bg-indigo-600 text-white rounded-md text-sm font-medium hover:bg-indigo-700"
                  >
                    Save
                  </button>
               </div>
